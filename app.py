...
<body>
    ...
    <script>
        // При загрузке страницы проверяем, сохранял ли пользователь ник ранее
        ...
        // Сохраняем ник перед отправкой формы
        function saveNick() {
            const nick = document.getElementById('nickname').value.strip();
...
def create():
    ...
    if text:
        ...
        # Если ник пустой, генерируем Аноним + ID на основе его IP-адреса
        if not nickname:
            user_ip = request.headers.get('X-Forwarded-For', request.remote_addr) or "127.0.0.1"
            # Создаем короткий хэш (4 символа) из IP
